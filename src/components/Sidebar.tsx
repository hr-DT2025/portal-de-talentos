...
export default function Sidebar() {
  ...
  // Helper para estilos de navegación activos/inactivos
  ...
  const isHR = user?.role === Role.HR || user?.role === Role.SUPERADMIN || user?.role === Role.DIRECTOR; 
  // Nota: Ajusté la condición isHR ligeramente para que los directores también vean el menú administrativo si así lo deseas, 
  // si no, déjalo como estaba: const isHR = user?.role === Role.HR;
  ...
  return (
    <div className="flex flex-col h-full bg-onix">
      ...
      <nav className="flex-1 mt-4">
        ...
        {/* === NUEVO ENLACE: MI EXPEDIENTE === */}
        <NavLink to="/my-file" className={navClass}>
          <FolderOpen size={20} />
